...
Tent Selection Strategy:
    Among trials better than No-TTA, select the one with lowest accuracy
    (worst among better-than-baseline) for conservative evaluation.
    If none better than No-TTA, fallback to best accuracy.

EA-TTA Selection Strategy:
    For each CLIP variant (ViT-B/32, ViT-L/14), select trial with best accuracy.
...
def main():
    ...
    # Tent selection strategy: worst among better-than-baseline
    ...
    for clip_name in CLIP_MODELS:
        ...
        print(f"\n[EA-TTA-{clip_name}] Best trial: {best_ea['name']} acc={best_ea['accuracy'] * 100:.2f}%")
    ...
    for clip_name, info in results["cifar10c"]["ea_tta"].items():
        print(f"EA-TTA ({clip_name}) best : {info['best']['accuracy'] * 100:.2f}%")
