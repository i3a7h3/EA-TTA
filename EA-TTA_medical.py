...
def collect_per_image_probs(model_name, clip_tag,
                            wrapper_no, wrapper_tent, wrapper_eatta,
                            loader):
    """Collect per-image predictions and identify improvement cases."""
    names, ys = [], []
    p_no, p_te, p_ea = [], [], []
    ...
    with torch.no_grad():
        for x, y, name in tqdm(loader, desc="Per-image probs", leave=False):
            ...
            p2 = torch.sigmoid(wrapper_eatta(x)).item()
            names.append(name[0])
            ys.append(int(y.item()))
            p_no.append(float(p0))
            p_te.append(float(p1))
            p_ea.append(float(p2))
            del x

    loss_no = [bce_per_image(p, y) for p, y in zip(p_no, ys)]
    loss_te = [bce_per_image(p, y) for p, y in zip(p_te, ys)]
    loss_ea = [bce_per_image(p, y) for p, y in zip(p_ea, ys)]

    # Condition 1: EA-TTA better than both No TTA and Tent
    cond1_idx = [
        i for i in range(len(names))
        if (loss_ea[i] < loss_no[i] and loss_ea[i] < loss_te[i])
    ]
    # Condition 2: Tent better than No TTA, EA-TTA better than Tent
    cond2_idx = [
        i for i in range(len(names))
        if (loss_te[i] < loss_no[i] and loss_ea[i] < loss_te[i])
    ]
        ...
        if rows:
            ...
            with open(csv_path, "w", newline="") as f:
                w = csv.DictWriter(
                    f,
                    fieldnames=[
                        "filename","label","prob_no_tta","prob_tent","prob_eatta",
                        "loss_no_tta","loss_tent","loss_eatta"
                    ],
                )
                w.writeheader()
    ...
    dump(cond1_idx, "cond1_EATTA_better_than_both")
    dump(cond2_idx, "cond2_Tent_better_NoTTA_EATTA_best")

    return names, ys, p_no, p_te, p_ea, loss_no, loss_te, loss_ea
...
def main():
    ...
    for model_name, clip_tag in COMBOS:
        ...
        # Per-image analysis
        names, ys, p_no, p_te, p_ea, l_no, l_te, l_ea = collect_per_image_probs(
            model_name, clip_tag,
            base_model, tent_wrapper, eatta_wrapper, eval1_loader
        )
        ...
        for idx, (x, y, fname) in enumerate(
            ...
            info = (
                f"y{yv}_"
                f"pNO{p_no[idx]:.3f}_pTE{p_te[idx]:.3f}_pEA{p_ea[idx]:.3f}_"
                f"lNO{l_no[idx]:.3f}_lTE{l_te[idx]:.3f}_lEA{l_ea[idx]:.3f}"
            )
            ...
            ea_path   = dir_ea   / f"{Path(name).stem}__{info}.png"
